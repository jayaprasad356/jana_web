...
if (isset($_GET['table']) && $_GET['table'] == 'users') {
    ...
    foreach ($res as $row) {
        ...
        $operate = '<a href="users.php?id=' . $row['id'] . '" title="View">View User</a>';
...
if (isset($_GET['table']) && $_GET['table'] == 'news_jobs') {
    $offset = 0;
    $limit = 10;
    $where = '';
    $sort = 'id';
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $search = $db->escapeString($_GET['search']);
        $where .= "WHERE company_name like '%" . $search . "%' OR job_title like '%" . $search . "%'";
    }
    if (isset($_GET['sort'])){
        $sort = $db->escapeString($_GET['sort']);
    }
    ...
    $sql = "SELECT COUNT(`id`) as total FROM `news_jobs` ";
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];

    $sql = "SELECT * FROM news_jobs ". $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $view = '<a href="view-news_jobs.php?id=' . $row['id'] . '" class="label label-primary" title="View">View</a>';
        $operate = '<a href="edit-news_jobs.php?id=' . $row['id'] . '" title="View"><i class="fa fa-edit"></i>Edit</a>';
        
        $tempRow['id'] = $row['id'];
        $tempRow['job_title'] = $row['job_title'];
        $tempRow['company_name'] = $row['company_name'];
        $tempRow['company_address'] = $row['company_address'];
        $tempRow['email'] = $row['email'];
        $tempRow['phone_no'] = $row['phone_no'];
        $tempRow['interview_date'] = $row['interview_date'];
        $tempRow['interview_time'] = $row['interview_time'];
        $tempRow['emp_qualification'] = $row['emp_qualification'];
        $tempRow['emp_experience'] = $row['emp_experience'];
        $tempRow['link'] = $row['link'];
        $tempRow['venue'] = $row['venue'];
        $tempRow['salary'] = $row['salary'];
        $tempRow['more_details'] = $row['more_details'];
        $tempRow['language'] = $row['language'];
        $tempRow['operate'] = $operate;
        $tempRow['operate1'] = $view;
        $rows[] = $tempRow;
    }
    $bulkData['rows'] = $rows;
    print_r(json_encode($bulkData));
...
if (isset($_GET['table']) && $_GET['table'] == 'companies') {
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $search = $db->escapeString($_GET['search']);
        $where .= "WHERE company_name like '%" . $search . "%' OR job_title like '%" . $search . "%'OR location like '%" . $search . "%'OR salary like '%" . $search . "%'";
    }
    if (isset($_GET['sort'])){
        $sort = $db->escapeString($_GET['sort']);
    }
    ...
    $sql = "SELECT * FROM companies " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $operate = ' <a href="edit-company.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i></a>';
        //$operate = ' <a class="btn-xs btn-danger" href="delete-category.php?id=' . $row['id'] . '"><i class="fa fa-trash-o"></i>Delete</a>';
        ...
        $tempRow['salary'] = $row['salary'];
        if(!empty($row['image'])){
            $tempRow['image'] = "<a data-lightbox='company' href='" . $row['image'] . "' data-caption='" . $row['image'] . "'><img src='" . $row['image'] . "' title='" . $row['image'] . "' height='50' /></a>";
        }else{
            $tempRow['image'] = 'No Image';
